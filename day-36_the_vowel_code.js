...
function encode(string) {
  ...
  return chars
    .map((char) => {
      if (vowelsNumber.hasOwnProperty(char)) {
        return vowelsNumber[char];
      }
      return char;
    })
    .join("");
...
function decode(string) {
  ...
  return chars
    .map((char) => {
      let index = Object.values(vowelsNumber).indexOf(Number(char));
      if (index < 0) {
        return char;
      }

      return Object.keys(vowelsNumber)[index];
    })
    .join("");
}
